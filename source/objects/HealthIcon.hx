...
class HealthIcon extends FlxSprite
{
	...
	public function changeIcon(char:String, ?allowGPU:Bool = true, ?iconOffsetsPar:Array<Float>, ?scalePar:Float = 1, ?flipXpar:Bool = false, ?blendThing:String = '', ?fpsThing:Int = 24) {
		if(this.char != char)
		{
			...
			else
			{
				...
				animation.play(char);
				if (animation.curAnim.numFrames == 3) hasThirdIcon = true;
			}
			iconOffsets[0] = (width - 150) + iconOffsetsPar[0];
			iconOffsets[1] = (height - 150) + iconOffsetsPar[1];
			scale.set(scalePar, scalePar);
	...
	{ 
		switch (getCurrentAnimation())
		{
			...
			case 'toWinning': if (isAnimationFinished()) playAnimation('winning', 'idle');
			case 'fromLosing' | 'fromWinning': if (isAnimationFinished()) playAnimation('idle');
			case '': playAnimation('idle');
			default: playAnimation('idle');
		}
	}
