...
//waveform for everything by @inky03 from psych PR #15931 
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		//ENGINES EVENTS
		['CountDown', "Does the countdown in song\nValue 1: Which Countdown to Show (pre-ready, ready, set or go)\nValue 2: TRUE if need play sound or FALSE to not play"],
		['Hey!', "Plays the \"Hey!\" animation\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend, DAD = Only Dad\nSomething else = GF and BF.\nValue 2: Custom animation duration,\nleave it blank for 0.6s\nValue 3: Different Animation name instead of default \"Hey!\"\nLeave it blank to leave default"],
		['Set Character Speed', "Sets Character Speed on beat,\nValue 1: Which character speed to set (Dad, Bf, Gf)\nValue 2: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\n\nWarning: Value must be integer!\nValue 3: TRUE if you want Idle play FORCED with no interruption"],
		['Add Camera Zoom', "Used on MILF on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.03)\nLeave the values blank if you want to use Default."],
		['Focus Camera', 'Value 1:\nCharacter to focus on (default is dad) - Can be any character\nValue 2: "origin" Duration in steps (default 4.0)\nValue3: Ease - Any FlxEase, can be "INSTANT" or "CLASSIC"\n"CLASSIC" is by default.\nValue 4: Additional X and Y Camera Offsets'],
		['Set Camera Bop', 'How often does the camera bop on beat?\nValue 1 - The rate of camera bopping\nValue 2 - The intensity of camera bop\nValue 3 - Speed of Camera Bop lerp'],
		['Shake Beat', 'Makes camera shakes at beat, rate is same as camera bop one'],
		['Zoom Camera', 'Value 1:\nZoom Ease, Any FlxEase, can be "INSTANT" or "CLASSIC"\n"CLASSIC" is by default.\nValue 2: \nZoom (default 1.0)\nValue 3: Duration in steps (default 4.0)\nValue4: Mode (default "direct", anything else is "stage")'],
		...
		['Fade Camera', "Value 1: Color\nValue 2: Duration in steps\nValue 3: Camera (camGame, camHUD, camNotes, camOverlayHUD or camOther)\nValue 4: True if Fade In or False if Fade Out"],
		['Set Health', 'Value 1: Health value to set\nValue 2: Ease (can be empty if Linear)\nValue 3: Duration in steps (can be blank for INSTANT)'],
		['Add Health', 'Value 1: Health value to add or remove\nValue 2: Ease (can be empty if Linear)\nValue 3: Duration in steps (can be blank for INSTANT)'],
		...
		['Beat Drain', "health drain on step hit\n\nValue 1: On/Off\nValue 2: Amount, 0.023 is default (same health you gain from hitting a 'sick' note)\nValue 3: Step Number (8 is default)"],
		['Solid Graphic Behind Characters', "Value 1: Alpha, Value 2: Duration in steps (can be 0 or null for INSTANT), Value 3: Color in hex (can be blank for BLACK)"],
		['Subtitles', "Value 1: Text Display\nValue 2: Duration in steps (Default: 4 steps)\nValue 3: Color (by hex: 0xFFFF0000 or by character icon (dad, bf, gf))\nValue 4: Size (Default: 24), Outline size (Default: 1.25)\nValue 5: Intro (Example: bop, fade in, fly away), End (Example: fade out or nothing), Font (default: vcr)"],
		['Set Character Position', 'Value 2: Char\nValue 2: X Y\n Value 3: Duration in steps\n Value 4: Ease'],
		['Set Character Color', 'Value 1: Char\nValue 2: Color in hex\n Value 3: Duration in steps\n Value 4: Ease'],
		...
		['Set Character Color Transform', 'Value 1: Character color to change\nValue 2: Red, Green, Blue, alpha offset\nValue 3: Red, Green, Blue, alpha multiplier\n Value 4: Duration in steps\n Value 5: Ease'],
		['Set Character Visibility', "Value 1: Character to change (dad, bf, gf)\nValue 2: How transparent the character is\nValue 3: Duration in steps\nValue 4: Ease"],
		['Add trail', 'Adds trail behind character\nValue 1: Trails length, delay, alpha, diff Variables (Example: 4, 24, 0.3, 0.069)\nValue 2: Which Character should get trail (DAD, BF, GF)\nValue 3: Color in hex (can be blank for no change)\nValue 4: Blend (Can be blank if no blend)'],
		...
		['Update Vocals Volume', "Value 1: Bf (main) vocal volume\nValue 2 Dad vocal volume (if exists)"],
		['Change Character', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"],
		['Change Icon', "Value 1: Character icon to change (Dad, BF, GF)\nValue 2: New icon's name\nValue 3: Options for icon (flipX true or false, scale, offset x, offset y, blend, fps (if icon is animated)\nLeave blank for default)Value 4: RGB Color for icon (Leave blank for characters default)\nExample: 255, 133, 49"],
		...
		['Change Combo Camera', "Value 1: Is it should be on game camera or ui camera? (camHUD or camGame)\nValue 2: X and Y position\nExample: 650, 300"],
		['Singing Shakes', 'Value 1: Turn on (true or false)\nValue 2: Which character shake'],
		['Change Note Camera Move Offset', "Value 1: Camera move offset float"],
		['Show YOU text under strums', "Shows text YOU under your strums"],
		['Change Visual Time Length', 'Value 1: Time length in seconds (Leave it blank for current length)\nValue 2: Duration in steps (leave it blank to be instant)\nValue 3: Tween Ease'],
		['Update Strum Position Variable', "Sets current note position for variable curPlayerStrumX, curPlayerStrumY\nand curOpponentStrumX, curOpponentStrumY\nUseful for modchart scripts things if needs"],
		['Change Object zIndex layer', "Changes object's zIndex layer layer\nValue 1: Object name (boyfriend, dad, gf or etc)\nValue 2: zIndex layer number"],
		...
		#else
		['Dadbattle Spotlight', "Used in Dad Battle,\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF"] //Here due to fact that stage will be there anyways
		#end
	];
	...
	function onChartLoaded()
	{
		...
		postfixInputText.text = PlayState.SONG.postfix;
	...
	function onSelectNote()
	{
		if(selectedNotes.length == 1) //Only one note selected
		{
			...
			if(!note.isEvent) //Normal note
			{
				if(!note.isEvent)
				{
					...
					holdTimerStepper.value = note.customSingTime;
					animSuffixInputText.text = note.animSuffix;
					altHeyInputText.text = note.heyAnim;
					lightStrumCheck.checked = note.lightStrum;
				...
				else
				{
					...
					holdTimerStepper.value = 0;
					animSuffixInputText.text = '';
					altHeyInputText.text = '';
		...
		else if(selectedNotes.length > 1)
		{
			...
			holdTimerStepper.value = selectedNotes[0].customSingTime;
			animSuffixInputText.text = selectedNotes[0].animSuffix;
			altHeyInputText.text = selectedNotes[0].heyAnim;
			lightStrumCheck.checked = selectedNotes[0].lightStrum;
	...
	var animSuffixInputText:PsychUIInputText;
	var altHeyInputText:PsychUIInputText;
	var holdTimerStepper:PsychUINumericStepper;

	var lightStrumCheck:PsychUICheckBox;
	var noAnimCheck:PsychUICheckBox;

	var ghostsDropDown:PsychUIDropDownMenu;
	var ghostTypes:Array<String> = ['', 'Default Ghost', 'Arrow Movement Ghost', 'Ascend Ghost', 'Fall Ghost', 'Left Velocity Ghost', 'Right Velocity Ghost', 'Left and Right Velocity Ghost', 'Random Left and Right Velocity Ghost'];
	
	var catchNoteCheck:PsychUICheckBox;
	...
	function addNoteTab()
	{
		...
		holdTimerStepper.onValueChange = function()
		{
			...
			var durs:Float = selectedNotes[0].customSingTime;
			for (note in selectedNotes)
			{
				if(note == null || note.isEvent) continue;
				
				note.setSingDur(holdTimerStepper.value + (note.customSingTime - durs));
			}
			softReloadNotes();
		};
		...
		altHeyInputText = new PsychUIInputText(objX, objY, 150, '', 8);
		altHeyInputText.onChange = function(old:String, cur:String)
		{
			if(selectedNotes.length < 1) return;
			...
			for (note in selectedNotes)
			{
				...
				note.setHeyAnim(cur);
				if(cur.trim().length < 1) note.setHeyAnim('');
			}
			softReloadNotes();
		}
		...
		lightStrumCheck = new PsychUICheckBox(objX + 175, objY, 'Light Strum?', 80, function()
		{
			...
			for (note in selectedNotes)
			{
				...
				note.setStrumLight(lightStrumCheck.checked);
			}
			softReloadNotes();
		});
		...
		noAnimCheck = new PsychUICheckBox(objX + 175, objY, 'No Animation?', 80, function()
		{
			...
			for (note in selectedNotes)
			{
				...
				note.setNoAnim(noAnimCheck.checked);
			}
			softReloadNotes();
		});
		...
		catchNoteCheck = new PsychUICheckBox(objX, objY, 'Catch Note? (For Opponent)', 80, function()
		{
			if(selectedNotes.length < 1) return;
			...
			for (note in selectedNotes)
			{
				...
				note.setCatchNote(catchNoteCheck.checked);
			}
			softReloadNotes();
		});
		...
		missAnimCheckBox = new PsychUICheckBox(objX + 175, objY, 'No Miss Animation?', 80, function()
		{
			...
			for (note in selectedNotes)
			{
				...
				note.setMissAnim(missAnimCheckBox.checked);
			}
			softReloadNotes();
		});
		...
		invisibleCheck = new PsychUICheckBox(objX, objY, 'Make Note Invisible?', 80, function()
		{
			if(selectedNotes.length < 1) return;
			...
			for (note in selectedNotes)
			{
				...
				note.setInvisible(invisibleCheck.checked);
			}
			softReloadNotes();
		});
		...
		tab_group.add(new FlxText(holdTimerStepper.x, holdTimerStepper.y - 15, 200, 'Custom Sing Duration:'));
		tab_group.add(new FlxText(animSuffixInputText.x, animSuffixInputText.y - 15, 100, 'Alt Anim Suffix:'));
		tab_group.add(new FlxText(altHeyInputText.x, altHeyInputText.y - 15, 200, 'Alt Hey Anim (if Hey note type):'));
		tab_group.add(new FlxText(ghostsDropDown.x, ghostsDropDown.y - 15, 200, 'Ghost Type (None if no Ghost):'));
		tab_group.add(susLengthStepper);
		tab_group.add(strumTimeStepper);
		tab_group.add(holdTimerStepper);
		tab_group.add(animSuffixInputText);
		tab_group.add(altHeyInputText);
		tab_group.add(lightStrumCheck);
		tab_group.add(noAnimCheck);
	...
	function addSectionTab()
	{
		...
		mustHitCheckBox = new PsychUICheckBox(objX, objY, 'Player Focus (Works only for GF section).', 80, function()
		{
			var sec = getCurChartSection();
	...
	function reloadNotesDropdowns()
	{
		...
		// Ghost type drop down
		if(ghostsDropDown != null)
		{
			var displayGhostTypes:Array<String> = ghostTypes.copy();

			for (id => key in displayGhostTypes)
			{
				if(id == 0) continue;
				displayGhostTypes[id] = '$id. $key';
			}

			var lastSelected:String = ghostsDropDown.selectedLabel;
			ghostsDropDown.list = displayGhostTypes;
			ghostsDropDown.selectedLabel = lastSelected;
		}
	...
	var postfixInputText:PsychUIInputText;
	...
	function addSongTab()
	{
		...
		postfixInputText = new PsychUIInputText(objX, objY + 40, 100, '', 8);
		postfixInputText.onChange = function(old:String, cur:String) PlayState.SONG.postfix = cur;

		allowVocalsCheckBox = new PsychUICheckBox(objX, objY + 60, 'Allow Vocals', 80, function()
		{
		...
		tab_group.add(new FlxText(postfixInputText.x, postfixInputText.y - 15, 0, 'Custom PostFix/Variation:'));
		tab_group.add(songNameInputText);
		tab_group.add(postfixInputText);
	...
	function addFileTab()
	{
		...
		if(SHOW_EVENT_COLUMN)
		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save Events...', function()
			{
				...
				updateChartData();
				fileDialog.save('events.json', PsychJsonPrinter.print({events: PlayState.SONG.events, format: 'gold_v1'}, ['events']),
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			...
			fileDialog.open(function()
			{
				...
				try
				{
					...
					{
						loadedChart.format = 'gold_v1_convert';
						Song.convertGold(loadedChart);
						File.saveContent(fileDialog.path, PsychJsonPrinter.print(loadedChart, ['sectionNotes', 'events']));
						showOutput('Updated "$filePath" from format "$fmt" to "gold_v1" successfully!');
					}
					else if(!fmt.startsWith('gold_v1'))
					{
	...
	var launchEditorPlay:Bool = false;
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		if(launchEditorPlay) setChartTheme(false);
