...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		set('version', MainMenuState.goldFunkVersion.trim());
		...
		//MOTE rework group thing if it will break cuz uhh yeah
		Lua_helper.add_callback(lua, "getObjectOrder", function(obj:String/*, ?group:String = null*/) {
			var split:Array<String> = obj.split('.');
			var leObj:FlxObject = LuaUtils.getObjectDirectly(split[0]);
			if(split.length > 1) {
		...
		//MOTE rework if it will break cuz uhh yeah
		...
		Lua_helper.add_callback(lua, "setCameraFollowPoint", function(x:Float, y:Float) game.cameraFollowPoint.setPosition(x, y));
		...
		Lua_helper.add_callback(lua, "addCameraFollowPoint", function(?x:Float = 0, ?y:Float = 0) {
			game.cameraFollowPoint.x += x;
			game.cameraFollowPoint.y += y;
		});
		...
		Lua_helper.add_callback(lua, "getCameraFollowX", () -> game.cameraFollowPoint.x);
		Lua_helper.add_callback(lua, "getCameraFollowY", () -> game.cameraFollowPoint.y);
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, ?inFront:Bool = false) {
			...
			else
			{
				if(PlayState.instance == null || !PlayState.instance.isDead)
					...
					GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.boyfriend_death), mySprite);
			}
		});
